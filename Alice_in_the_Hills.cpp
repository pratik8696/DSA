...
int solve(vector<vector<int>> &mat, int i, int j)
{
    ...
    for (auto child : adj[hsh[{i, j}]])
    {
        dp[i][j] = max(dp[i][j], 1 + solve(mat, nodes[child].first, nodes[child].second));
    }
    return dp[i][j];
}
...
{
    maxPath = 0, n = sz(matrix), m = sz(matrix[0]);
...
void solver()
{
    ...
    forn(i, n)
    {
        ...
        forn(j, m)
        {
            res.pb({arr[i][j], hsh[{i, j}]});
        }
        sort(all(res));
        forn(j, m - 1)
        {
            adj[res[j].second].pb(res[j + 1].second);
        }
    }
    forn(j, m)
    {
