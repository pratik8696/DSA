...
void solve()
{
    ...
    ll tree[4 * v.size()];
    v64 temp;
    fill(al(tree, 4 * v.size()), 0);
    fill(al(res, v.size()), 0);
    ll xx = 0;
    for (auto t : v)
    {
        // cout << t.first << " " << t.second << endl;
        temp.pb(t.first);
        res[xx++] = t.second - t.first + 1;
    }
    build(res, tree, 0, v.size() - 1, 1);
    for (auto t : v)
    {
        ll idx = upper_bound(all(temp), t.second) - temp.begin();
        ll res1 = t.second - t.first + 1;

        // cout << res1 << " " << idx << " ";
        if (idx != temp.size())
        {
            res1 += (query(res, tree, 0, v.size() - 1, 1, idx, v.size() - 1));
        }
        // cout<<endl;
        ans = max(res1, ans);
    }
