...
void queryy(ll tree[], ll k, ll tn, ll l, ll r)
{
    if (l == r)
    {
        dbg(l);
    ...
    if (tree[2 * tn] >= k)
    {
        queryy(tree, k, 2 * tn, l, mid);
    }
    else
    {
        queryy(tree, k - tree[2 * tn], (2 * tn) + 1, mid + 1, r);
    }
}
...
void solve()
{
    ...
    forn(i, q)
    {
        ...
        if (opt == 1)
        {
            update(arr, tree, 0, n - 1, 1, x, 1 - arr[x]);
        }
        else
        {
            x++;
            queryy(tree, x, 1, 0, n - 1);
        }
    }
}
