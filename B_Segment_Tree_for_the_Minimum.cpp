...
void update(ll arr[], node tree[], ll s, ll e, ll tn, ll idx, ll value)
{
    if (s == e)
    {
        arr[idx] = value;
    ...
    if (idx > mid)
    {
        update(arr, tree, mid + 1, e, (2 * tn) + 1, idx, value);
    }
    else
    {
        update(arr, tree, s, mid, 2 * tn, idx, value);
    }
    tree[tn] = merge(tree[2 * tn], tree[(2 * tn) + 1]);
}
...
void solve()
{
    ...
    while (q--)
    {
        ...
        {
            node curr = query(arr, tree, 0, n - 1, 1, a, b - 1);
            cout << curr.val << endl;
        }
    }
}
