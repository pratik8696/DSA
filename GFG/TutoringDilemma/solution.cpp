...
int f(int n, vector<vector<int>> a, vector<vector<int>> b)
{
    ...
    for (auto t : b)
    {
        brr.push_back({t[0], t[1]});
    }
    sort(begin(arr), end(arr), compare);
    sort(begin(brr), end(brr), compare);
    int cc = 0, i = 0;
    pbds s;
    for (int j = 0; j < n; j++)
    {
        while (i < n && arr[i].first < brr[j].first)
        {
            s.insert(arr[i++].second);
        }
        auto it = s.upper_bound(brr[j].second);
