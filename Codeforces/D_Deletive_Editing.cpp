    ...
    void solve()
    {
        ...
        // DONATIN
        // now pb the elements equal to the count from back
        for (auto t : cc)
        {
            ll val_req = t.se;
            ll char_use = t.fi;
            auto vec = v[char_use];
            for (ll i = vec.size() - 1, j = 1; i >= 0 && j <= val_req; i--, j++)
            {
                res.pb({char_use, vec[i]});
            }
        }
        // now sorting the vector and making the string
