...
double sum(ll idx, ll rem, ll value)
{
    if (rem == value)
    {
        return 0;
    ...
    double ans = -1e15;
    // take it
    ans = max(ans, sum(idx + 1, rem + 1, value) + (powers[rem] * arr[idx]));
    ans = max(ans, sum(idx + 1, rem, value));
    // dpp[idx][rem] = 1;
    dp[idx][rem] = ans;
    return ans;
}
...
void solve()
{
    ...
    arr.resize(n);
    powers.resize(n);
    vector<double> summer(n);
    powers[n - 1] = 1.0;
    for (ll i = n - 2; i >= 0; i--)
    {
        powers[i] = powers[i + 1] * 0.9;
        powers[i] = round(powers[i] * 1e12) / 1e12;
    }
    reverse(all(powers));
    forn(i, n)
    {
        summer[i] += (i - 1 >= 0 ? summer[i - 1] : 0) + powers[i];
    }
    forn(i, n)
    {
        cin >> arr[i];
    ...
    for (ll i = 1; i <= n; i++)
    {
        double curr = 1200 / sqrtl(i);
        double val = sum(0, 0, i) / summer[i - 1];
        val -= curr;
        dbg(val);
        ans = max(ans, val);
    }
