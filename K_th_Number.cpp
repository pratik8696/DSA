...
ll query(ll s, ll e, ll tn, ll l, ll r, ll value)
{
    ll mid = (s + e) / 2;
    // out
    if (s > r || l > e)
    ...
    if (s >= l && r >= e)
    {
        int idx = upper_bound(all(treee[tn]), value) - begin(treee[tn]);
        return idx;
    }

    ll ans1 = query(s, mid, 2 * tn, l, r, value);
    ll ans2 = query(mid + 1, e, (2 * tn) + 1, l, r, value);
    return ans1 + ans2;
}
...
void solve()
{
    ...
    while (q--)
    {
        ...
        cin >> a >> b >> c;
        a--, b--;
        ll i = -1e10, j = 1e10, ans = 0;
        while (i <= j)
        {
            ll mid = i + (j - i) / 2;
            if (query(0, n - 1, 1, a, b, mid) >= c)
            {
                j = mid - 1;
                ans = mid;
            }
