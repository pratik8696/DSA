...
// here we are considering array values as indexes
// now if we need to find the maximum subsequence which is found till arr[i]-k to arr[i]
// then we can find that using segment tree , we will find the maximum element in the range from arr[i]-k to arr[i]
// and similary we can do for all the indices
// as we find the max element in the range
...
void solve()
{
    ...
    forn(i, n)
    {
        ll left = max(arr[i], 0ll);
        ll right = min(arr[i], 1e5);
    }
    // map<int, int> m;
    // forn(i, n)
    // {
    ...
    //         if (m[arr[i] - j] > 0)
    //         {
    //             curr_max = max(m[arr[i] - j] + 1, curr_max);
    //         }
    //     }
