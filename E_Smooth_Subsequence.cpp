...
void build(v64 &arr, vector<Tree> &tree, ll s, ll e, ll tn)
{
    if (s == e)
    {
        tree[tn].v.pb(arr[s]);
    ...
    merge(tree[2 * tn].v, tree[(2 * tn) + 1].v, tree[tn].v);
}

ll query(v64 &arr, vector<Tree> &tree, ll s, ll e, ll tn, ll l, ll r, ll k)
{
    ll mid = (s + e) / 2;
    // out
    if (s > r || l > e)
    ...
    if (s >= l && r >= e)
    {
        ll res = upper_bound(all(tree[tn].v), k) - begin(tree[tn].v);
        return sz(tree[tn].v) - res;
    }

    ll ans1 = query(arr, tree, s, mid, 2 * tn, l, r, k);
    ll ans2 = query(arr, tree, mid + 1, e, (2 * tn) + 1, l, r, k);
    return ans1 + ans2;
}
...
public:
    ...
    ll lca_sum_query(ll a, ll b)
    {
        ...
        while (head[a] != head[b])
        {
            ...
            sum += query(0, idx - 1, 1, pos[head[a]], pos[a]);
            a = parent[head[a]];
        }
        if (depth[a] < depth[b])
        {
        ...
        sum += query(0, idx - 1, 1, pos[b], pos[a]);
        return sum;
    }
...
void solve()
{
    ...
    hld.init();
    dbg(hld.lca(10, 11));
    dbg(hld.lca_sum_query(10, 12));
}



int treee[int(2e6 + 10)];
...
void solve()
{
    ...
    forn(i, n)
    {
        ll left = max(arr[i] - d, 0ll);
        ll right = min(arr[i] + d, ll(5e5));
        ll res = query(0, 5e5 + 10, 1, left, right);
        res++;
        // cout << arr[i] << " " << res << endl;
        update(0, 5e5 + 10, 1, arr[i], res);
        ans = max(ans, res);
    }
    dbg(ans);
}
