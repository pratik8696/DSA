...
void build(ll arr[], node tree[], ll s, ll e, ll tn)
{
    if (s == e)
    {
        tree[tn].v.push_back(arr[s]);
        return;
    }
    ll mid = (s + e) / 2;
    build(arr, tree, s, mid, 2 * tn);
    build(arr, tree, mid + 1, e, (2 * tn) + 1);
    merge(all(tree[2 * tn].v), all(tree[(2 * tn) + 1].v), back_inserter(tree[tn].v));
}
...
ll query(ll arr[], node tree[], ll s, ll e, ll tn, ll l, ll r, ll val)
{
    ...
    if (s >= l && r >= e)
    {
        return lower_bound(all(tree[tn].v), val) - tree[tn].v.begin();
    }
...
void solve()
{
    ...
    forsn(i, 1, n)
    {
        cout << i - query(arr, tree, 0, n - 1, 1, 0, i - 1, arr[i]) << " ";
    }
    cout << endl;
}
