...
void solve()
{
    ...
    while (m--)
    {
        ...
        {
            ll steps = abs(c);
            ll left = lvl[a] - lvl[lca];
            // cout << left << "..." << steps << ln;
            if (steps > left)
            {
                ll rem_steps = steps - left;
                ll from_left = (lvl[b] - lvl[lca]) - rem_steps;
                // cout << from_left << " " << rem_steps << ln;
                steps = from_left;
                while (steps)
                {
                    // a needs to come up
                    b = sparse[b][__lg(steps)];
                    steps -= fastexpo(2, __lg(steps));
                }
                cout << b << ln;
            }
