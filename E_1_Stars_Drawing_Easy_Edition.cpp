...
void solve()
{
    ...
    vv64 vertical(n + 3, v64(m + 3));
    vv64 horizontal(n + 3, v64(m + 3));
    vv64 a;
    forsn(i, 1, n + 1)
    {
        forsn(j, 1, m + 1)
        {
            if (arr[i][j] == '*')
            {
                ...
                {
                    ll up = max(i - ans, 1ll);
                    ll down = min(n + 1, i + ans + 1);
                    ll left = max(1ll, j - ans);
                    ll right = min(m + 1, j + ans + 1);
                    vertical[up][j]++;
                    vertical[down][j]--;
                    horizontal[i][left]++;
                    horizontal[i][right]--;
    ...
    forsn(i, 1, n + 1)
    {
        forsn(j, 1, m + 1)
        {
            f[i][j] = horizontal[i][j] + vertical[i][j];
            f[i][j] = min(1ll, f[i][j]);
            if (arr[i][j] == '*' && f[i][j] == 0)
            {
                dbg("-1");
