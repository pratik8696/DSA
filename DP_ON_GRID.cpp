...
ll minsum(vv64 &arr, ll row, ll col, ll n, ll m)
{
    if (row == n - 1)
    {
        return arr[row][col];
    ...
    ll up = MOD, down = MOD;
    up = minsum(arr, row + 1, col, n, m);
    down = minsum(arr, row + 1, col + 1, n, m);
    return arr[row][col] + min(up, down);
}
