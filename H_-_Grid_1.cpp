...
int fun(int x, int y, vector<vector<char>> &arr, vector<vector<int>> &dp)
{
    if (x == 1 && y == 1)
    {
        return 1;
    ...
    {
        left = fun(x - 1, y, arr, dp) % MOD;
    }
    if (isvalid(x, y - 1, arr))
    {
        up = fun(x, y - 1, arr, dp) % MOD;
    }
    dp[x][y] = up + left;
    return dp[x][y] % MOD;
}

int mazeObstacles(int n, int m, vector<vector<char>> &arr)
{
    vector<vector<int>> dp(n + 10, vector<int>(m + 10, -1));
    nn = n;
    mm = m;
    return fun(n, m, arr, dp);
}
